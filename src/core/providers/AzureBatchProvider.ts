 ...
 *
 * Implements batch processing using Azure OpenAI Batch API
 */
export class AzureBatchProvider implements BatchProvider {
  private client: OpenAI;
  private logger: JobLogger;

  constructor(jobId: string) {
    this.client = AzureConfig.getClient();
    this.logger = new JobLogger(`Azure:${jobId}`);
  }
    ...
    try {
      ...
      return {
        ...
        requestCounts: response.request_counts
          ...
          : undefined,
        errors: response.errors?.data?.map((err: any) => ({
